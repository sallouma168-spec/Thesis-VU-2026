...
Reads: data/articles_with_questions.csv. Writes: data/framed_prompts.csv.
"""
import pandas as pd

df = pd.read_csv("data/articles_with_questions.csv")
...
framed_df.to_csv("data/framed_prompts.csv", index=False)
print(f"Generated {len(framed_df)} framed prompts from {len(df)} articles x 3 frames")
print("\nExample (article 0, all 3 frames):")
example = framed_df[framed_df["article_id"] == framed_df["article_id"].iloc[0]]
for _, r in example.iterrows():
    print(f"  [{r['frame_type']}] {r['framed_question']}")