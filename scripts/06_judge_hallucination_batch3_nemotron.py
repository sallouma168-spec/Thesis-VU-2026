"""Pipeline stage 4: hallucination judge — classify answers SUPPORTED/UNSUPPORTED/CONTRADICTORY using the CORRECTED judge (EXCLUDED Nemotron responses).

Reads: data/framed_prompts_batch3.csv, data/rag_responses_nemotron_batch3.jsonl. Writes: data/hallucination_labels_batch3_nemotron.jsonl.
"""
import os, json, time
import pandas as pd
...
prompts_df = pd.read_csv("data/framed_prompts_batch3.csv")
text_lookup = prompts_df.drop_duplicates(subset=["article_id"]).set_index("article_id")["text"].to_dict()

OUTPUT_PATH = "data/hallucination_labels_batch3_nemotron.jsonl"
